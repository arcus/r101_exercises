...
high_cholesterol_females <- cirrhosis %>%
  filter(Sex == "F",
         Cholesterol >= 300) %>%
  select(ID, Status, Stage, Age, Cholesterol, Bilirubin)
```
...
```{r}
# First I'll check if "New_Age" looks right, just putting the ages together
high_cholesterol_females <- cirrhosis %>%
  filter(Sex == "F",
         Cholesterol >= 300) %>%
  select(ID, Status, Stage, Age, Cholesterol, Bilirubin) %>%
  mutate(New_Age = round(Age/365.25,2)) %>%
  select(ends_with("Age"))
```

I mean, I guess that looks right?  I do a spot check on a couple if I'm nervous.
