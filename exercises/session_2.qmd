...
Take a look at the *Files* pane on the bottom right (make sure you're looking within the "exercises" directory). Note that there is a file named *covid_testing.csv*. The objective of this exercise is to *import*, or *ingest* that CSV file into R as a *data frame*, and then take a first look at its contents using the *Data Viewer*.
...
Run the following code chunk. If you get an error that says something like `there is no package called 'tidyverse'`, that means that tidyverse isn't installed. Run the code chunk above to get it installed, then keep going!
